import { CMC_KEY, CMC_ENDPOINT, DL_ENDPOINT, CGK_ENDPOINT } from '../helper/constants.js';
import Utils from '../helper/utils.js';

export default class PriceApi {

  static async getCoins() {
    return Utils.doSimpleRequest(CMC_ENDPOINT + '/cryptocurrency/quotes/latest?CMC_PRO_API_KEY=' + CMC_KEY + '&symbol=BTC,THETA,TFUEL,TDROP', (data) => {
  ...
  static async getCoinsFromCGK() {
    return Utils.doSimpleRequest(CGK_ENDPOINT + '/api/v3/coins/markets?vs_currency=usd&ids=bitcoin,theta-fuel,theta-token,thetadrop&price_change_percentage=24h,1y', (data) => {
      const body = data
      const coins = {};
  ...
  static async getTVL() {
    return Utils.doSimpleRequest(DL_ENDPOINT + '/charts/theta', (data) => {
